...
app.MapGet("/course/{courseId:guid}/lessons", async (Guid courseId, HttpContext httpContext, EnrollmentDbContext db, CatalogApiClient catalogApiClient) =>
{
    var userIdStr = httpContext.Request.Headers["X-User-Id"].FirstOrDefault();
    if (!Guid.TryParse(userIdStr, out var userId)) return Results.Unauthorized();
    ...
    var materials = await catalogApiClient.GetCourseMaterialsAsync(courseId, httpContext.Request.Headers);
    if (materials == null) return Results.NotFound("Course materials not found or access denied by catalog.");

    return Results.Ok(materials);
});

app.MapGet("/course/{courseId:guid}/lesson/{lessonId:guid}/content", async (Guid courseId, Guid lessonId, HttpContext httpContext, EnrollmentDbContext db, CatalogApiClient catalogApiClient, IHttpClientFactory httpClientFactory) =>
{
    var userIdStr = httpContext.Request.Headers["X-User-Id"].FirstOrDefault();
    if (!Guid.TryParse(userIdStr, out var userId)) return Results.Unauthorized();
    ...
    var materials = await catalogApiClient.GetCourseMaterialsAsync(courseId, httpContext.Request.Headers);
    if (materials == null) return Results.NotFound();
